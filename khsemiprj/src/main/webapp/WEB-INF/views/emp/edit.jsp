        ...
        $.ajax({
            ...
            success: function (response){
                if(response === true){ 
                    ...
                    $wrapper.find(".fail-feedback").text("이미 존재하는 이메일입니다.");
                    state.empEmailValid = false; 
                } else { 
                    $("[name=empEmail]").removeClass("success fail").addClass("success");
                    $wrapper.find(".success-feedback").text("사용 가능한 이메일입니다.");
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response === true) {
                    ...
                    if(certFailCount >= 5){
                        $(".cert-message").text("인증번호를 5회 이상 틀렸습니다. 다시 인증번호를 전송 후 시도해주세요.").css("color", "red");
                        $(".field-cert").prop("disabled", true);
                        $(".btn-cert-check").prop("disabled", true);
                    } else {
                        $(".cert-message").text("인증번호가 일치하지 않습니다. (" + certFailCount + "/5회 오류)").css("color", "red");
                    }
                }
            }
        });
...
<script type="text/template" id="cert-template">
    <div class="cert-wrapper flex-area" style="flex-wrap: wrap;">
        ...
        <div class="cert-message w-100 mt-5"></div>
    </div>
</script>
	...
	<form action="edit" method="post" enctype="multipart/form-data" class="form-check">
		...
		<div class="cell mt-30">
            <div class="gray mb-10">
                <b>이메일</b> <span style="font-size: 13px; color: #666;">(기존: ${empDto.empEmail})</span>
            </div>
	    	<div class="flex-area" style="flex-wrap: wrap;">
		        <input type="text" name="empEmail" class="field flex-fill" inputmode="email" placeholder="새로운 이메일을 입력하세요">
	            ...
	            <div class="fail-feedback w-100 mt-5"></div> 
            </div>
	    </div>
