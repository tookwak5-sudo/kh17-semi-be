...
public class ReplyDislikeDao {
	...
	public void insert(String empId, long replyNo) {
		String sql = "insert into reply_dislike(emp_id, reply_no) values(?, ?)";
		Object[] params = {empId, replyNo};
		jdbcTemplate.update(sql, params);
	}
	
	public boolean delete(String empId, long replyNo) {
		String sql = "delete reply_dislike where emp_id=? and reply_no=?";
		Object[] params = {empId, replyNo };
		return jdbcTemplate.update(sql, params) > 0;
	}
	
	public boolean check(String empId, long replyNo) {
		String sql = "select count(*) from reply_dislike where emp_id=? and reply_no=?";
		Object[] params = { empId, replyNo };
		return jdbcTemplate.queryForObject(sql, long.class, params) > 0;
	}
	
	public long count(long replyNo) {
		String sql = "select count(*) from reply_dislike where reply_no=?";
