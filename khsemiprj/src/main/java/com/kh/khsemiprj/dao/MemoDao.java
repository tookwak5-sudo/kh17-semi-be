	...
	public class MemoDao {
		...
		private Set<String> allowColumns = Set.of("memo_sender_id", "memo_title", "memo_content", "memo_type");
	    ...
	    public List<MemoDto> selectList(String receiverId, PageVO pageVO) {
	        ...
	        String sql = "select * from ("
	                        + "select rownum rn, TMP.* from ("
	                            + "select M.*, E.emp_name "
	                            + "from memo M "
	                            + "inner join emp E on M.memo_sender_id = E.emp_id "
	                            + "where M.memo_receiver_id = ? "
	                            //+ "and instr(" + column + ", ?) > 0 "
	                            + "and (instr(" + (column.equals("M.memo_sender_id") ? "M.memo_sender_id" : column)  + ", ?) > 0 "
	                            + " or instr(" + (column.equals("M.memo_sender_id") ? "E.emp_name" : column) + ", ?) > 0)"
	                            + "order by M.memo_no desc"
	                        + ") TMP"
	                    + ") where rn between ? and ?";
	 	...
	 	public int count(String receiverId, PageVO pageVO) {
	 		...
	 		String sql = "select count(*) from memo m "
	 					+ "INNER JOIN emp e ON e.emp_id = m.memo_sender_id "
	 					+ "where memo_receiver_id = ? "
	 					//+ "and instr("+pageVO.getColumn()+", ?) > 0";
	 					+ "and (instr(" + (pageVO.getColumn().equals("memo_sender_id") ? "m.memo_sender_id" : pageVO.getColumn())  + ", ?) > 0 "
                        + " or instr(" + (pageVO.getColumn().equals("memo_sender_id") ? "e.emp_name" : pageVO.getColumn()) + ", ?) > 0)";
	 		Object[] params = { receiverId, pageVO.getKeyword(), pageVO.getKeyword() };
